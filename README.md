...
## 🚀 CONNEXION LOCALE → RAILWAY + SUPABASE
...
# 2. Configurer Supabase (script interactif)
./configure-supabase.sh
# → Coller la connection string depuis Supabase Dashboard
# → Le script teste et configure automatiquement
...
**URLs :**
- Backend Local : http://localhost:7001
- Frontend Local : http://localhost:7101
- Swagger : http://localhost:7001/swagger-ui.html
...
- **Configuration Supabase** : `CONNEXION_SUPABASE_INSTRUCTIONS.md`
- **Résumé complet** : `RESUME_CONFIGURATION.md`

### 🔧 Scripts Utiles

- `./configure-supabase.sh` - Configuration interactive de Supabase
- `./test-db-connection.sh` - Tester la connexion à la base de données
- `./mvnw spring-boot:run` - Démarrer le backend
...
### 🚀 Démarrage rapide (sans authentification)
...
./run-local.sh
```
Le backend démarre en mode DEV avec authentification **désactivée** sur http://localhost:7001
...
npm run dev
```
Le frontend démarre sur http://localhost:7101

**Accès :**
- 🌐 **Application** : http://localhost:7101
...
- ✅ **Backend sans auth** - Tous les endpoints accessibles sans token
- ✅ **CORS configuré** pour localhost:7101
- ✅ **Logs SQL activés**
...
📖 **Guide complet** : Voir `DEV_MODE_GUIDE.md`

### Configuration avancée

**Backend :**
- Profil : `dev` (défini dans `run-local.sh`)
- Port : 7001 (modifiable via `SERVER_PORT`)
- Base de données : PostgreSQL sur localhost:5432
- Configuration : `src/main/resources/application-dev.yml`

**Frontend :**
- Port : 7101 (défini dans `vite.config.ts`)
- Proxy API : http://localhost:7001 (automatique)
- Variables d'env : `frontend/.env.local`
