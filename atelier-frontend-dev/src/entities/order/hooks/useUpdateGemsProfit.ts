...
const useUpdateGemsProfit = (orderId: number) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (profitPercentage: number) => OrdersApi.updateOrderGemsProfit(orderId, profitPercentage),
    onSettled: () => {
...
export default useUpdateGemsProfit;
