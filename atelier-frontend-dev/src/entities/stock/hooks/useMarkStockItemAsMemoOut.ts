...
const useMarkStockItemAsMemoOut = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ orderId, dto }: { orderId: number; dto: MarkStockItemMemoOutDto }) =>
      StockApi.markStockItemAsMemoOut(orderId, dto),

    onMutate: updateStockItemSnapshots(StockItemStatus.MEMO_OUT, queryClient),
...
export default useMarkStockItemAsMemoOut;
