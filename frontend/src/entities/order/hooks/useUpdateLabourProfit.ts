...
const useUpdateLabourProfit = (orderId: number) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (profitPercentage: number) => OrdersApi.updateOrderLabourProfit(orderId, profitPercentage),
    onSettled: () => {
...
export default useUpdateLabourProfit;
